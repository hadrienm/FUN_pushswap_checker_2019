...
checkSndPart [] = True
checkSndPart (x:[])
    | last (x:[]) == "-" || last (x:[]) == "+" || last (x:[]) == "*" || last (x:[]) == "/" || last (x:[]) == "%" = False
checkSndPart (x:xs)
    | isStringDigit x == True = checkSndPart xs
    | last (x:xs) == "-" || last (x:xs) == "+" || last (x:xs) == "*" || last (x:xs) == "/" || last (x:xs) == "%" = False
    | otherwise = False
...
sortList l a b
    ...
    | otherwise = sortList (drop 1 l) (sndRotateFunction a) (sndRotateFunction b)

verifieList :: [String] -> IO () --Verifier que la liste est bien triée dans l'odre croissant
verifieList a
    | a!!0 == "failed" = print "KO"
verifieList (x:[]) = print "OK"
verifieList (x:xs)
    | x > head xs = print "KO"
